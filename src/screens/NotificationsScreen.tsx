...
export const NotificationsScreen = ({ onBack, onNavigate }: NotificationsScreenProps) => (
    <div className="screen-shell screen-shell-sm min-h-screen bg-paper">
        ...
        <div className="space-y-0 border border-border">
            {NOTIFICATIONS.map(n => (
                <div key={n.id} className={`p-5 sm:p-8 border-b border-border last:border-0 ${n.read ? "bg-paper/30 opacity-60" : "bg-white shadow-[inset_4px_0_0_0_#BC9F8B]"}`}>
                    <div className="flex gap-4 sm:gap-8 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-2">
                                <h4 className="text-lg font-serif italic font-bold leading-tight break-words">{n.title}</h4>
                                <span className="text-[9px] text-muted uppercase font-bold tracking-widest shrink-0">{n.date}</span>
                            </div>
