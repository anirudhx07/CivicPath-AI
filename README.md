...
## PWA Notes
- CivicPath AI now ships as a mobile-installable PWA with a service worker generated by the Vite PWA plugin.
- The app shell, static assets, manifest, icons, offline fallback page, and bundled lesson content are precached for offline use.
- Lesson data lives in the app bundle, so it is available offline without any extra network request.
- Gemini API responses are not cached by the service worker. Live AI responses stay network-bound unless the app is using its local fallback text.
- The app uses `public/manifest.json` plus placeholder SVG icons so it can be installed from mobile browsers.
